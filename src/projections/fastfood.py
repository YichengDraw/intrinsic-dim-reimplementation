...
class FastfoodProjection(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, seed: int = 42):
        ...
        # After transform: Var[output_i] should ≈ 1/d
        # FWHT scales by factor of padded_dim per application (unnormalized)
        # Two HWT applications: scale by padded_dim^2
        # Multiply by G (variance 1): scale by padded_dim^2
        # We want output variance ≈ 1/d, so scale by 1/(padded_dim * sqrt(d))
        S = torch.ones(self.padded_dim) / (self.padded_dim * math.sqrt(input_dim))
